...
{
    MCMEnsureState();
    // Bypass signing check — allow MCM container access
    NSLog(@"[MCMFilza] Container access: %@", MCMSignedCodeIdentifier());
...
void MCMFilzaStart(void)
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        // Bypass signing check — allow any identifier to use MCM
        NSString *actual = MCMSignedCodeIdentifier();
        NSLog(@"[MCMFilza] identifier: %@ (bypassing check)", actual);
